 ...
 * Provides immediate synchronous baseline data and dynamic JSON loading.
 * Eliminates all asynchronous blocking delays during game bootstrap.
 */
...
export const PORTFOLIO_CONTENT = {
  ...
  sectors: {
    ...
    sector2: {
      ...
      links: [
        { label: "Read RoboCup@Work Article & Video", url: "pages/article-robocup.html" },
    ...
    sector5: {
      ...
      links: [
        { label: "Read Partner Dancing & WSDC Competition Article", url: "pages/article-dancing.html" },
        { label: "Explore Oslo Travel & Photo Gallery", url: "pages/article-oslo.html" },
...
export async function initPortfolioContent() {
  try {
    ...
    const res = await fetch(`${prefix}assets/data/portfolio.json`);
    if (!res.ok) return PORTFOLIO_CONTENT;
    const data = await res.json();
    
    if (data.header) PORTFOLIO_CONTENT.header = data.header;
    if (data.sectors) PORTFOLIO_CONTENT.sectors = data.sectors;
    if (Array.isArray(data.subItems) && data.subItems.length > 0) PORTFOLIO_CONTENT.subItems = data.subItems;
    if (Array.isArray(data.socialLinks) && data.socialLinks.length > 0) PORTFOLIO_CONTENT.socialLinks = data.socialLinks;
    
    return PORTFOLIO_CONTENT;
  } catch (err) {
    // Graceful fallback: synchronous baseline data already in place
    return PORTFOLIO_CONTENT;
  }
}
